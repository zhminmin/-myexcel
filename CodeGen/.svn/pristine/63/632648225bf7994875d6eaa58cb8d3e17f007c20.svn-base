...
<mapper namespace="nameSpace_${tableInfo.className}">
	...
	<sql id="PAGING_END">
       	<![CDATA[
       	) t1 ) WHERE linenum <= ${r'#{endNum}' } 
       		 and linenum > ${r'#{beginNum}' } 
       	]]>
	</sql>
	...
	<sql id="SELECT_PARAMETER">
	    <where>
	    	...
	    	<if test="${column.attrname}Begin != null and ${column.attrname}Begin != ''">
		    	<![CDATA[ and to_char(t.${column.columnName },'yyyy/MM/dd') >= ${r'#{'}${column.attrname }Begin${r'}'} ]]>
		    </if>
		    <if test="${column.attrname}End != null and ${column.attrname}End != ''">
		  		<![CDATA[ and to_char(t.${column.columnName },'yyyy/MM/dd') <= ${r'#{'}${column.attrname}End${r'}'} ]]>
		    </if>
	...
	<delete id="deleteByExample" parameterType="${tableInfo.classname}">
		delete from ${tableInfo.tableName} t
