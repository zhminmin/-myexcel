...
function ${tableInfo.classname}_add( ) {
	parent.addNewTab('新增${tableInfo.tableComment }', mainActionUrl + '?method=add');
}

function ${tableInfo.classname}_edit( obj ) {
	var p = $( obj ).parent();
	var id = $( p ).attr("ids");
	parent.addNewTab('编辑${tableInfo.tableComment }', mainActionUrl + '?method=edit&${tableInfo.primaryKey.attrname }=' + id);
}
